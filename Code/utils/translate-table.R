...
{
    raw_data <- DBI::dbReadTable(con, x)
    cb <- codebookFromDB(con, name, qv, vc)
    d <- raw2translated(raw_data, cb, cleanse_numeric = cleanse_numeric)
    d
}
...
.dbqTableVars <- paste0(
    'SELECT "Variable" AS "Variable" FROM %s ', # Metadata.QuestionnaireVariables
    'WHERE "TableName" = \'%s\''
)
...
.dbqTableCodebook <- paste0(
    ...
    'WHERE "TableName" = \'%s\''  # FIXME Table or TableName ? change in metadata load process
)
